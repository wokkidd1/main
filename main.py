...
async def admin_panel(m: Message):
    ...
    text = (f"⚙️ **Админка**\nЮзеров: `{u_count}`\nДоход Stars: `{int(stars)}`🌟 | USDT: `{usdt}`$\n\n"
            f"Команды:\n`/broadcast текст` — рассылка\n`/give_stars ID кол-во`\n`/set_premium ID дни`\n`/ban ID` / `/unban ID` ")
    await m.answer(text, parse_mode="Markdown")
...
async def handle_link(m: Message):
    if not await check_sub(m.from_user.id): return await m.answer(f"⚠️ Подпишись на канал: {CHANNEL_URL}")
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=])
    await m.answer("🎯 Выберите режим:", reply_markup=kb)
...
async def preset_call(call: CallbackQuery):
    ...
    if res[4] == 1: return await call.answer("Вы забанены!", show_alert=True)
    ...
    if not is_prem and dl >= FREE_LIMIT:
        ...
        else: return await call.message.edit_text("❌ Лимит исчерпан! Купите Premium или пакет видео.")
    else:
    ...
    except: await call.message.answer("❌ Ошибка при обработке!")
