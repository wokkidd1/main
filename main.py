...
# Временное хранилище ссылок (защита от лимита 64 байта в кнопках)
user_links = {}
...
async def cmd_start(m: Message):
    ...
    if not db_query("SELECT user_id FROM users WHERE user_id = ?", (uid,), fetchone=True):
        db_query("INSERT INTO users (user_id, downloads, last_reset, join_date, extra_limits) VALUES (?, 0, ?, ?, 0)", (uid, today, today), commit=True)
    await m.answer(f"🚀 Бот готов! Твой ID: `{uid}`", reply_markup=get_main_kb(uid), parse_mode="Markdown")
...
async def cmd_shop(m: Message):
    await m.answer("🛒 **Выберите тариф:**", reply_markup=get_shop_kb())
